...
const toolTipCat = document.querySelector(".category")
console.log(toolTipCat)

// Event functions (bar hover effekt)
const handleHover = (d) => {
  toolTipCat.innerHTML = d.question_label
  let selectedName = d.question_label
  d3.selectAll(".bars")
    .transition()
    .duration(200)
    .style("opacity", (d2) => {
      return d2.question_label === selectedName ? 1 : 0.4
    })
...
function draw(csv) {
    ...
    gradients.append('stop').attr('offset', '55%').attr('stop-color', '#609839');
  
    gradients.append('stop').attr('offset', '50%').attr('stop-color', 'yellow');
  
    gradients.append('stop').attr('offset', '100%').attr('stop-color', 'red');
  ...
  // Hvit sirkel / underkategorier
    svg
      .append('circle')
      .attr('r', maxBarHeight + 40)
      .classed('question-circle', true);

  // Grå sirkel, ser den nesten ikke
    svg
      .append('circle')
      .attr('r', maxBarHeight)
  ...
  // Inner radius sirkel
    svg.append('circle').attr('r', innerRadius).classed('center-circle', true);
    ...
    d3.csv(csv, function (error, data) {
      ...
      var question_text = svg
        .selectAll('path.question_label_arc')
        .data(data)
        .enter()
        .append('path')
      ...
      question_text.each(function (d, i) {
        ...
        if (
          ...
          d3.select(textPath.childNodes[0]).attr(
            'dy',
      ...
      var bars = svg
        ...
        .on("mouseover", (data) => {
          ...
          return tooltip.style("visibility", "visible").text('Dette er data: ' + data.value)
        })
        .on('mousemove', () => {
          return tooltip.style('top', (event.pageY-150)+'px').style('left',(event.pageX-1000)+'px')
        });
      ...
      svg
        .selectAll('circle.x.minor')
        .data(ybis_scale.ticks(10))
        .enter()
        .append('circle')
        .attr('stroke', 'red')
        .classed('gridlines minor color', true)
        .attr('r', function (d) {
          return x_scale(d);
        });
      ...
      // category lines
      // Kan fjernes for å lage hel sirkel
      svg
    ...
    text.each(function (d) {
      var text = d3.select(this),
        ...
        tspan = text
          ...
          .attr('dy', dy + 'em'),
        arcLength =
          ((d.endAngle - d.startAngle) / (2 * Math.PI)) * (2 * Math.PI * radius),
